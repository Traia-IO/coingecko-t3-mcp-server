# Coingecko_t3 MCP Server Implementation Guide

You are working on implementing the Coingecko_t3 MCP (Model Context Protocol) server. The basic structure has been set up, and your task is to implement the actual API integration.

## API Information

- **API Name**: Coingecko_t3
- **Documentation**: []()
- **Website**: [https://pro-api.coinmarketcap.com/](https://pro-api.coinmarketcap.com/)
- **Authentication**: Required - API key via `COINGECKO_T3_API_KEY` environment variable
...
When a client provides their own Coingecko_t3 API key:
...
Flow:
  1. Client connects with their Coingecko_t3 API key
  2. MCP server uses client's API key to call Coingecko_t3 API
  3. No payment required
  4. Client pays Coingecko_t3 directly (not the MCP server)
```

**Use Case**: Clients who already have a Coingecko_t3 subscription/API key
...
Flow:
  ...
  4. MCP server uses its INTERNAL Coingecko_t3 API key to call the API
  5. Client pays the MCP server (not Coingecko_t3)
```

**Use Case**: Pay-per-use clients without their own Coingecko_t3 subscription
...
async def your_tool_name(context: Context, param1: str) -> Dict[str, Any]:
    ...
    if api_key:
        ...
        api_key_to_use = os.getenv("COINGECKO_T3_API_KEY")
...
**Required**:
- `COINGECKO_T3_API_KEY`: Server's internal Coingecko_t3 API key (used when clients pay via 402)
...
COINGECKO_T3_API_KEY=your_coingecko-t3_api_key_here
...
```json
{
  "mcp_server": {
    "capabilities": [
      ...
      "search_coingecko_t3",
      "get_coingecko_t3_info",
      // Add all other tools you implement
    ]
  },
  "tags": ["coingecko_t3", "api", /* add relevant tags like "search", "data", etc. */]
}
```
...
Add functions to call the Coingecko_t3 API with retry support. Example pattern:
...
def call_coingecko_t3_api(endpoint: str, params: Dict[str, Any], api_key: str) -> Dict[str, Any]:
    """Call Coingecko_t3 API endpoint with automatic retry."""
...
"Content-Type": "application/json"
    ...
    try:
        ...
        raise Exception(f"Coingecko_t3 API error: {str(e)}")
```
...
```python
@mcp.tool()
async def search_coingecko_t3(
    context: Context,
    query: str,
    limit: int = 10
) -> Dict[str, Any]:
    """
    Search Coingecko_t3 for [specific data type].
    ...
    try:
        # The call_coingecko_t3_api function already has retry logic
        results = call_coingecko_t3_api(
...
```python
@mcp.tool()
async def get_coingecko_t3_info(
    context: Context,
    item_id: str
) -> Dict[str, Any]:
    """
...
```python
@mcp.tool()
async def create_coingecko_t3_item(
    context: Context,
    name: str,
    data: Dict[str, Any]
) -> Dict[str, Any]:
    """
    Create a new item in Coingecko_t3.
...
3. **Add Tool Examples** in README.md:
   ...
   result = await tool.search_coingecko_t3(query="example", limit=5)
   ```
...
### 9. Common Coingecko_t3 Use Cases

Based on the API documentation, consider implementing tools for these common use cases:

1. **TODO**: List specific use cases based on Coingecko_t3 capabilities
2. **TODO**: Add more relevant use cases
3. **TODO**: Include any special features of this API

### 10. Example API Calls

Here are some example API calls from the Coingecko_t3 documentation that you should implement:
...
- Check the Coingecko_t3 API documentation: 
- Review the MCP specification: https://modelcontextprotocol.io
- Look at other MCP server examples in the Traia-IO organization

Remember: The goal is to make Coingecko_t3's capabilities easily accessible to AI agents through standardized MCP tools. 