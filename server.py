#!/usr/bin/env python3
"""
Coingecko_t3 MCP Server - FastMCP with D402 Transport Wrapper
...
- COINGECKO_T3_API_KEY: Server's internal API key (for paid requests)
...
logger = logging.getLogger('coingecko-t3_mcp')
...
API_KEY = os.getenv("COINGECKO_T3_API_KEY")
if not API_KEY:
    logger.warning(f"⚠️  COINGECKO_T3_API_KEY not set - payment required for all requests")

logger.info("="*80)
logger.info(f"Coingecko_t3 MCP Server (FastMCP + D402 Wrapper)")
...
mcp = FastMCP("Coingecko_t3 MCP Server", host="0.0.0.0")
...
# - COINGECKO_T3_API_KEY: Server's internal API key for payment mode
#
...
async def coinmarketcap_id_map_0pfp(
    context: Context,
    listing_status: str = "active",
    slug: Optional[str] = None,
    start: int = 1,
    ...
    Example Usage:
        await coinmarketcap_id_map_0pfp()
    ...
    try:
        ...
        logger.error(f"Error in coinmarketcap_id_map_0pfp: {e}")
...
async def coinmarketcap_id_map_dusa(
    context: Context,
    start: int = 1,
    limit: Optional[int] = None,
    sort: str = "id",
    ...
    Example Usage:
        await coinmarketcap_id_map_dusa()
    ...
    try:
        ...
        logger.error(f"Error in coinmarketcap_id_map_dusa: {e}")
...
async def quotes_historical_jnw4(
    context: Context,
    time_start: Optional[str] = None,
    time_end: Optional[str] = None,
    count: float = 10,
    ...
    try:
        ...
        logger.error(f"Error in quotes_historical_jnw4: {e}")
...
async def quotes_latest_ode0(
    ...
    try:
        ...
        logger.error(f"Error in quotes_latest_ode0: {e}")
...
def create_app_with_middleware():
    ...
    app.add_middleware(
        ...
        server_name="coingecko-t3-mcp-server"  # MCP server ID for tracking
    )
    ...
    async def health_check(request: Request) -> JSONResponse:
        ...
        return JSONResponse(
            content={
                "status": "healthy",
                "service": "coingecko-t3-mcp-server",
...
if __name__ == "__main__":
    logger.info("="*80)
    logger.info(f"Starting Coingecko_t3 MCP Server")
    logger.info("="*80)
