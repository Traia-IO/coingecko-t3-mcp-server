...
# Script to build and run the Coingecko_t3 MCP Server locally in Docker
...
IMAGE_NAME="coingecko-t3-mcp-server"
CONTAINER_NAME="coingecko-t3-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running Coingecko_t3 MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# Coingecko_t3 MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO

# API Authentication (server's internal key for payment mode)
COINGECKO_T3_API_KEY=
...
if ! grep -q "^COINGECKO_T3_API_KEY=[^[:space:]]" .env; then
    if [ -t 0 ]; then
        echo ""
        echo -e "${YELLOW}📝 Please enter your Coingecko_t3 API key:${NC}"
        echo -e "${YELLOW}   (This is the server's internal API key used when clients pay via 402)${NC}"
        read -p "COINGECKO_T3_API_KEY: " API_KEY_VALUE
        if [ -n "$API_KEY_VALUE" ]; then
            if grep -q "^COINGECKO_T3_API_KEY=" .env; then
                if [[ "$OSTYPE" == "darwin"* ]]; then
                    sed -i '' "s|^COINGECKO_T3_API_KEY=.*|COINGECKO_T3_API_KEY=$API_KEY_VALUE|" .env
                else
                    sed -i "s|^COINGECKO_T3_API_KEY=.*|COINGECKO_T3_API_KEY=$API_KEY_VALUE|" .env
                fi
            else
                echo "COINGECKO_T3_API_KEY=$API_KEY_VALUE" >> .env
            fi
        ...
        echo -e "${YELLOW}⚠️  Non-interactive mode: Please set COINGECKO_T3_API_KEY in .env manually${NC}"
    fi
else
    echo -e "${GREEN}✅ COINGECKO_T3_API_KEY already set in .env${NC}"
fi
...
echo
echo -e "${GREEN}✅ Coingecko_t3 MCP Server is running!${NC}"
echo
