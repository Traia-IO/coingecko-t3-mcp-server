# Coingecko_t3 MCP Server

This is an MCP (Model Context Protocol) server that provides with authentication via Bearer tokens access to the Coingecko_t3 API. It enables AI agents and LLMs to interact with Coingecko_t3 through standardized tools.

## Features

- 🔧 **MCP Protocol**: Built on the Model Context Protocol for seamless AI integration
- 🌐 **Full API Access**: Provides tools for interacting with Coingecko_t3 endpoints
...
- **Coingecko_t3 Website**: [https://pro-api.coinmarketcap.com/](https://pro-api.coinmarketcap.com/)
- **API Documentation**: []()
...
*Note: Replace `example_tool` with actual Coingecko_t3 API tools based on the documentation.*
...
1. Clone this repository:
   ```bash
   git clone https://github.com/Traia-IO/coingecko-t3-mcp-server.git
   cd coingecko-t3-mcp-server
   ```

2. Set your API key:
   ```bash
   export COINGECKO_T3_API_KEY="your-api-key-here"
   ```
...
2. Run the server:
   ```bash
COINGECKO_T3_API_KEY="your-api-key-here" uv run python -m server
   ```
...
Clients with their own Coingecko_t3 API key can use the server for free:

```bash
# Request with client's API key
curl -X POST http://localhost:8000/mcp \
  -H "Authorization: Bearer CLIENT_COINGECKO_T3_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"method":"tools/call","params":{"name":"example_tool","arguments":{"query":"test"}}}'
```

**Flow**:
1. Client provides their Coingecko_t3 API key
2. Server uses client's API key to call Coingecko_t3 API
3. No payment required
4. Client pays Coingecko_t3 directly
...
6. Server uses its INTERNAL Coingecko_t3 API key to call the API
...
```bash
# Required
COINGECKO_T3_API_KEY=your_internal_coingecko-t3_api_key  # Server's API key (for payment mode)
...
1. **Client Decision**:
   - Has Coingecko_t3 API key? → Mode 1 (Authenticated)
...
1. Create API client functions for Coingecko_t3 endpoints
...
```json
{
  "github_url": "https://github.com/Traia-IO/coingecko-t3-mcp-server",
  "mcp_server": {
    "name": "coingecko-t3-mcp",
    "description": "Just another coinckecko",
...
- `COINGECKO_T3_API_KEY`: Your Coingecko_t3 API key (required)
## Troubleshooting
